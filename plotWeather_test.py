...
class WeatherPlot():
	...
	self.plotTime = 8		#Length of data plot, in hours
	...
	while timeCheck < self.plotTime:
	    ...
	    # Sort datafile into a dictionary of dictionaries
	    # data = {datetime object: {sensor: sensorData}}
		...
		if timeCheck < self.plotTime and len(datalist) <= 1:
		    ...
		    print "tempFile: /logs/"+logDate.strftime("%Y%m%d")+"-weather.txt"
		    templist = tempFile.readlines()
		    tempFile.close()
		    templist.reverse()
		    print len(datalist)
